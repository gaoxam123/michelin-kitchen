...
export const getLikes = async ({ blogId }) => {
  try {
    ...
    return console.log(error.response?.data?.message || "Failed to get users");
  }
};
...
export const addLike = async ({ userId, blogId }) => {
  try {
    const response = await request.post(`/likes`, {
      userId,
      blogId,
    ...
    return console.log(error.response?.data?.message || "Failed to like blog");
  }
};
...
export const removeLike = async ({ userId, blogId }) => {
  try {
    const response = await request.delete(`/likes`, {
      data: { userId, blogId },
    });
